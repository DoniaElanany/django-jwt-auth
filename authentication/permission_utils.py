...
def user_can_perform_action(user, page_name, action_name):
    ...
    Args:
        ...
        action_name: str - name of the action (e.g., 'save_draft', 'approve_invoice')
    ...
    try:
        page_action = PageAction.objects.get(page=page, name=action_name)
...
def get_user_page_permissions(user):
                    ...
                    'actions': [
                        {
                            ...
                            'display_name': str,
                            'allowed': bool,
                            'denial_id': int (optional, only if denied)
                        }
                    ]
                }
            ]
    ...
    if user.user_type.type_name == 'super_admin':
        ...
        pages = Page.objects.prefetch_related('actions').all()
        for page in pages:
            page_data = {
                'page_id': page.id,
                'page_name': page.name,
            ...
            for action in page.actions.all():
    ...
    job_role_pages = JobRolePage.objects.filter(
        job_role=user.job_role
    ).select_related('page').prefetch_related('page__actions')
    ...
    for jrp in job_role_pages:
        ...
        for action in page.actions.all():
            action_data = {
                ...
                'name': action.name,
                'display_name': action.display_name,
                'allowed': action.id not in denial_map
            }
            
            if action.id in denial_map:
                action_data['denial_id'] = denial_map[action.id]
...
def get_user_denied_actions(user):
    ...
    denials = UserActionDenial.objects.filter(
        user=user
    ).select_related('page_action__page').order_by('created_at')
    ...
    for denial in denials:
        denied_actions.append({
            ...
            'action_name': denial.page_action.name,
            'action_display_name': denial.page_action.display_name,
